...
export const CarouselDialog = (props: CarouselDialogProps) => {
  ...
  return (
    <Dialog open onClose={props.handleClose} PaperProps={{ className: classes.paper }} fullWidth>
      <div className={classes.container}>
        <Carousel
          ...
          renderArrowNext={(clickHandler: () => void, hasNext: boolean, label: string) =>
            hasNext ? (
              <span
                ...
                title={label}
                className={joinClassNames(classes.arrowBtn, classes.next, 'next-arrow')}>
                <ArrowCircleRight
                  size='54px'
                  variant='linear'
          ...
          renderArrowPrev={(clickHandler: () => void, hasprevious: boolean, label: string) =>
            hasprevious ? (
              <span
                onClick={() => {
                  clickHandler()
                ...
                title={label}
                className={joinClassNames(classes.arrowBtn, classes.previous, 'previous-arrow')}>
                <ArrowCircleLeft
                  size='54px'
                  variant='linear'
          ...
          {props.media.map((item, i) => (
            <Fragment key={item._id}>
              {item.isImage ? (
                <img src={item.path} key={item._id} className={classes.carouselImageItem} />
              ) : (
